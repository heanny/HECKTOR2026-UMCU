...
def build_densenet_volume_6ch(ct_xyz, pet_xyz, hard_seg_xyz, prob_t_xyz, prob_n_xyz):
    """[x,y,z] CT/PET + seg(0/1/2) + prob_T + prob_N -> [6,100,100,155] float32.

    Channels in canonical order: CT, PET, prob_T, prob_N, hard_T, hard_N.
    Matches C = task3/CT+PET+prob_T+prob_N+hard_T+hard_N (in_channels=6).
    prob_T/prob_N are softmax probs (already [0,1]); NOT normalized.
    They go through the SAME pad+resample as the hard channels.
    """
    ct_norm  = _normalize_ct(ct_xyz)
    pet_norm = _normalize_pet(pet_xyz)
    prob_t   = prob_t_xyz.astype(np.float32)      # softmax prob, no normalize
    prob_n   = prob_n_xyz.astype(np.float32)
    hard_t   = (hard_seg_xyz == 1).astype(np.float32)
    hard_n   = (hard_seg_xyz == 2).astype(np.float32)

    img = np.stack([ct_norm, pet_norm, prob_t, prob_n, hard_t, hard_n], axis=0)  # [6,x,y,z]

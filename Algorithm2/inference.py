# Docker: Task2=Option B stacking(RF+DL clin_hTN), Task3=Docker1 original(alpha=0.1)
...
def run():
    ...
    seg_crop = run_segmentation(ct_crop, pet_crop)
    write_segmentation(
    ...
    rfs_value = run_rfs(feat_dict, ehr, dl_risk)
...
def run_segmentation(ct_crop, pet_crop):
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        seg_arr = _SEG_PREDICTOR.predict_single_npy_array(
...
def run_rfs(feat_dict, ehr, dl_risk):
    ...
    cox        = joblib.load(str(RADIO_DIR / "cox_docker1.joblib"))
    mms        = joblib.load(str(RADIO_DIR / "cox_minmax_docker1.joblib"))
    ...
    rf_risk = float(np.mean([m.predict(X_sc)[0] for m in rfs_models]))

    Xr       = mms.transform(np.array([[rf_risk, dl_risk]]))
    cox_risk = float(cox.predict(Xr)[0])

    # Grand Challenge: higher score = better prognosis (flip risk sign)
    return -1.0 * cox_risk
